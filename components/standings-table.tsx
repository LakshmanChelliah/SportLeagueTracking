      ...
      <table className="w-full border-separate border-spacing-0">
        ...
        <tbody>
          {rows.map((row) => {
            ...
            return (
              <tr key={row.team} className={mine ? "bg-[#e8572a]/15" : undefined}>
                <td className={cn("border-b border-border px-2.5 py-2.5 text-left font-display text-[22px] text-dim max-md:py-3.5 max-md:text-lg", mine && "shadow-[inset_3px_0_0_#e8572a]")}>
                  {row.rank}
                </td>
      ...
      {rows.map((row) => (
        <li
          ...
          className={cn(
            ...
            you === row.team && "bg-[#e8572a]/15 shadow-[inset_3px_0_0_#e8572a]",
          )}
        >
